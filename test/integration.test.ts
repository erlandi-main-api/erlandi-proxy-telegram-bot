...
test("store persists owner, audit, wizard, and portal settings",async()=>{const dir=await mkdtemp(join(tmpdir(),"erlandi-bot-")),path=join(dir,"bot.sqlite");try{let store=await Store.open(path,123);assert.equal(store.getUser(123)?.role,"owner");await store.audit(123,"test.action","target");assert.equal(store.listAudit(1)[0].action,"test.action");await store.setSession(123,{operation:"create",step:"name",models:[]});assert.equal(store.getSession(123)?.operation,"create");await store.clearSession(123);assert.equal(store.getSession(123),null);const portal=JSON.stringify({version:1,welcomeText:"Public",homeRows:[],pages:[]});await store.setSetting("public_portal.v1",portal,123);store=await Store.open(path,123);assert.equal(store.getSetting("public_portal.v1"),portal);await store.deleteSetting("public_portal.v1");assert.equal(store.getSetting("public_portal.v1"),null)}finally{await rm(dir,{recursive:true,force:true})}});
